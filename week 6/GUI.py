...
class Gui(tk.Tk):
    ...
    def render(self):
        ...
        for row_index in range(self.__environment.get_height()):
            for col_index in range(self.__environment.get_width()):
                ...
                # Set the current agent colour
                color = self.__agent_colours.get(type(agent), "white")

                # Create a new Canvas object with appropriate attributes
                cell = tk.Canvas(self.grid_frame, width=20, height=20, bg=color, borderwidth=1, relief="solid")
    ...
    def __init_world(self):
        ...
        self.render()  # Initial rendering of the grid
